...
if args.probe_mode == 'linear':
    ...
    for i in range(epoch):
        ...
        # save model every 5 epochs (e.g., 5, 10, 15, ...)
        if (i + 1) % 5 == 0:
            save_path = f"/ssd/kdpark/dongjae/moment/moment_sleep/23_multi_linear_epoch{i+1}.pth"
            torch.save(model.state_dict(), save_path)
            print(f"Model saved at epoch {i+1} → {save_path}")
...
# def fit_svm(features, y, MAX_SAMPLES=1000):
#     nb_classes = np.unique(y, return_counts=True)[1].shape[0]
#     train_size = features.shape[0]
...
#         # 샘플 줄이기
#         if train_size > MAX_SAMPLES:
#             split = train_test_split(features, y, train_size=MAX_SAMPLES,
#                                      random_state=0, stratify=y)
#             features, y = split[0], split[2]

#         best_score, best_model = -np.inf, None
#         for p in tqdm(params, desc="GridSearch"):
#             clf = SVC(**p, probability=True)
#             score = np.mean(
#                 cross_val_score(clf, features, y, cv=3, n_jobs=1)
#             )
#             if score > best_score:
#                 best_score, best_model = score, clf.fit(features, y)
#         return best_model
